...
//api from open-meteo
const URL =
  "https://api.open-meteo.com/v1/forecast?latitude=62.4723&longitude=6.1549&current=temperature_2m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=Europe%2FBerlin";
        ...
        <div className="upcomingWeatherContainer">
          {upComingWeatherCodes.map((code, index) => (
            <div
              key={index}
              className={`${weekDay[
                (currentTime.getDay() + index) % 7
              ].toLowerCase()}container`}
            >
              <p>
                {weekDay[(currentTime.getDay() + index) % 7]
                  .toUpperCase()
                  .slice(0, 3)}
              </p>
